...
!##########################################################
SUBROUTINE find_mat(matrix,sizemat,element,ind,pos)
    USE module_icp
    IMPLICIT NONE

    ! Subroutine parameters
    INTEGER(ki), INTENT(IN) :: element,sizemat
    INTEGER(ki), INTENT(IN) :: matrix(sizemat,2)
    ...
    DO i=1,sizemat
       IF ((matrix(i,1).eq.element).or.(matrix(i,2).eq.element)) THEN
          ind = ind + 1
          pos(ind) = i
       END IF
       IF ((matrix(i,1).eq.0).or.(matrix(i,2).eq.0)) EXIT
    END DO
...
!##########################################################
SUBROUTINE find_mat2(matrix,sizemat,ele1,ele2,ind)
    USE module_icp
    IMPLICIT NONE
    ...
    ! ind = count( ((matrix(:,1)==ele1) .or. (matrix(:,2)==ele1)) .and. ((matrix(:,1)==ele2) .or. (matrix(:,2)==ele2)) )
    
    DO i=1,sizemat
       IF (((matrix(i,1).eq.ele1).or.(matrix(i,2).eq.ele1)) .and. ((matrix(i,1).eq.ele2).or.(matrix(i,2).eq.ele2)) ) THEN
          ind = i
          EXIT
       END IF
       IF ((matrix(i,1).eq.0).or.(matrix(i,2).eq.0)) EXIT
